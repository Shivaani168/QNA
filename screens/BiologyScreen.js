...
export default class BiologyScreen extends Component {
 ...
 getUserDetails = () => {
  ...
  fetchQuestions = () => {
    ...
    db.collection("questionGrade10").where('subject','==','biology').onSnapshot((snapshot) => {
      var allT = [];
      snapshot.docs.map((doc) => {
        var task = doc.data();
        task.id = doc.id;
  ...
  renderItem = ({ item }) => {
    return (
<TouchableOpacity onPress={()=>this.props.navigation.navigate('BioAnswerQuestion', {questionId:item.id,question:item.question})}>
  ...
  render() {
    ...
    alignItems: 'center',marginRight:RFValue(70),marginDown:RFValue(20),backgroundColor:" white"}}>
            <Image
                source={require('../assets/biologo.png')}
