...
export default class ChemistryScreen extends Component {
 ...
 getUserDetails = () => {
  ...
  fetchQuestions = () => {
    ...
    db.collection("questionGrade10").where('subject','==','chemistry').onSnapshot((snapshot) => {
      var allT = [];
      snapshot.docs.map((doc) => {
        var task = doc.data();
        task.id = doc.id;
  ...
  renderItem = ({ item }) => {
    return (
<TouchableOpacity onPress={()=>this.props.navigation.navigate('ChemAnswerQuestion', {questionId:item.id,question:item.question})}>
  ...
  render() {
    if (this.state.fontsLoaded) {
      ...
      return (
        ...
        <TouchableOpacity style={{margin:10}} onPress={() => this.props.navigation.navigate('G10Home')}>
    ...
    alignItems: 'center',marginRight:RFValue(10),marginDown:RFValue(20),backgroundColor:" white"}}>
            <Image
                source={require('../assets/chemlogo.png')}
