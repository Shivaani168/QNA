...
export default class PhysicsScreen extends Component {
 ...
 getUserDetails = () => {
  ...
  fetchQuestions = () => {
    ...
    db.collection("questionGrade10").where('subject','==','physics').onSnapshot((snapshot) => {
      var allT = [];
      snapshot.docs.map((doc) => {
        var task = doc.data();
        task.id = doc.id;
  ...
  renderItem = ({ item }) => {
    return (
<TouchableOpacity onPress={()=>this.props.navigation.navigate('PhysicsAnswerQuestion', {questionId:item.id,question:item.question})}>
  ...
  render() {
    ...
    alignItems: 'center',marginRight:RFValue(60),marginDown:RFValue(20),backgroundColor:" white"}}>
            <Image
