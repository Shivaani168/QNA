...
export default class MathsScreen extends Component {
 ...
 getUserDetails = () => {
  ...
  fetchQuestions = () => {
    ...
    db.collection("questionGrade10").where('subject','==','maths').onSnapshot((snapshot) => {
      var allT = [];
      snapshot.docs.map((doc) => {
        var task = doc.data();
        task.id = doc.id;
  ...
  renderItem = ({ item }) => {
    return (
<TouchableOpacity onPress={()=>this.props.navigation.navigate('MathsAnswerQuestion', {questionId:item.id,question:item.question})}>
  ...
  render() {
    ...
    alignItems: 'center',marginRight:RFValue(100),backgroundColor:" white"}}>
            <Image
