...
export default class G7ScienceScreen extends Component {
  constructor(props) {
    super(props);
    this.state = {
      fontsLoaded: false,
 ...
 getUserDetails = () => {
  ...
  fetchQuestions = () => {
    ...
    db.collection("questionGrade7").where('subject','==','science').onSnapshot((snapshot) => {
      var allT = [];
      snapshot.docs.map((doc) => {
        var task = doc.data();
        task.id = doc.id;
  ...
  renderItem = ({ item }) => {
    return (
<TouchableOpacity onPress={()=>this.props.navigation.navigate('G7ScienceAnswerQs', {questionId:item.id,question:item.question})}>
