...
export default class G8ScienceScreen extends Component {
  constructor(props) {
    super(props);
    this.state = {
      fontsLoaded: false,
 ...
 getUserDetails = () => {
  ...
  fetchQuestions = () => {
    ...
    db.collection("questionGrade8").where('subject','==','science').onSnapshot((snapshot) => {
      var allT = [];
      snapshot.docs.map((doc) => {
        var task = doc.data();
        task.id = doc.id;
  ...
  renderItem = ({ item }) => {
    return (
<TouchableOpacity onPress={()=>this.props.navigation.navigate('G8ScienceAnswerQs', {questionId:item.id,question:item.question})}>
  ...
  render() {
    if (this.state.fontsLoaded) {
      ...
      return (
        ...
        <TouchableOpacity style={{margin:10}} onPress={() => this.props.navigation.navigate('G8Home')}>
       ...
       <TouchableOpacity style={{
         ...
         margin:RFValue(8)
          ...
          onPress={() => this.props.navigation.navigate('G8AddQuestion')}
          >
    ...
    alignItems: 'center',marginRight:RFValue(70),marginDown:RFValue(20),backgroundColor:" white"}}>
            <Image
                source={require('../assets/sciencelogo.png')}
