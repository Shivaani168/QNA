...
export default class G9ScienceScreen extends Component {
  constructor(props) {
    super(props);
    this.state = {
      fontsLoaded: false,
 ...
 getUserDetails = () => {
  ...
  fetchQuestions = () => {
    ...
    db.collection("questionGrade9").where('subject','==','science').onSnapshot((snapshot) => {
      var allT = [];
      snapshot.docs.map((doc) => {
        var task = doc.data();
        task.id = doc.id;
  ...
  renderItem = ({ item }) => {
    return (
<TouchableOpacity onPress={()=>this.props.navigation.navigate('G9ScienceAnswerQs', {questionId:item.id,question:item.question})}>
...
<View style={{flexDirection:"row",justifyContent:"space-between"}}>
        <TouchableOpacity style={{margin:10}} onPress={() => this.props.navigation.navigate('G9Home')}>
       ...
       <TouchableOpacity style={{
         ...
         margin:RFValue(8)
          ...
          onPress={() => this.props.navigation.navigate('G9AddQuestion')}
          >
