...
export default class G9EnglishScreen extends Component {
  constructor(props) {
    super(props);
    this.state = {
      fontsLoaded: false,
 ...
 getUserDetails = () => {
  ...
  fetchQuestions = () => {
    ...
    db.collection("questionGrade9").where('subject','==','english').onSnapshot((snapshot) => {
      var allT = [];
      snapshot.docs.map((doc) => {
        var task = doc.data();
        task.id = doc.id;
  ...
  renderItem = ({ item }) => {
    return (
<TouchableOpacity onPress={()=>this.props.navigation.navigate('G9EnglishAnswerQs', {questionId:item.id,question:item.question})}>
