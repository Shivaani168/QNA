...
export default class G9MathsScreen extends Component {
  constructor(props) {
    super(props);
    this.state = {
      fontsLoaded: false,
 ...
 getUserDetails = () => {
  ...
  fetchQuestions = () => {
    ...
    db.collection("questionGrade9").where('subject','==','maths').onSnapshot((snapshot) => {
      var allT = [];
      snapshot.docs.map((doc) => {
        var task = doc.data();
        task.id = doc.id;
  ...
  renderItem = ({ item }) => {
    return (
<TouchableOpacity onPress={()=>this.props.navigation.navigate('G9MathsAnswerQs', {questionId:item.id,question:item.question})}>
